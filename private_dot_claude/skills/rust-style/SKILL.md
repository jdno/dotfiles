---
name: rust-style
description: Rust coding style guide. Apply automatically when writing or modifying Rust code. Enforces let-else for early returns, variable shadowing, newtypes, explicit matching, and minimal comments.
---
...
Use `let ... else` to extract values and exit early on failure. This keeps the
...
Use `if let` only when the `Some`/`Ok` branch is short and there's no else
branch.
...
Shadow variables through transformations. Avoid prefixes like `raw_`, `parsed_`,
`trimmed_`.
...
Wrap strings in newtypes to add semantic meaning and prevent mixing different
string types. Use the [typed-fields](https://crates.io/crates/typed-fields)
crate to generate newtypes easily.
...
Always match all variants explicitly to get compiler errors when variants are
added.
...
Use full `match` expressions instead of `matches!`. Full matches provide better
compiler diagnostics when the matched type changes.
...
Destructure structs and tuples explicitly to get compiler errors when fields
change.
